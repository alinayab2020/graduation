(() => {
  ...
  const liteFX = reduceMotion || saveData || slowNetwork || (mobile && (deviceMemory <= 4 || cpuCores <= 4));
  const FILM_FPS = 15;
  ...
  const sceneIndexNumber = $('#sceneIndexNumber');
  ...
  function warmChapterImages() {
    const task = () => {
      if (saveData || slowNetwork) { hydratePlate(1); hydratePlate(2); return; }
      for (let scene = 1; scene <= 5; scene++) hydratePlate(scene);
    };
    if ('requestIdleCallback' in window) requestIdleCallback(task, { timeout: 1400 });
    else setTimeout(task, 420);
  }
  ...
  function renderFX(scene, local) {
    if (reduceMotion) {
      stage.style.setProperty('--fx-alpha', scene === 0 ? '.24' : '.38');
      return;
    }
    const direction = scene % 2 ? 1 : -1;
    const x = direction * lerp(-2.4, 3.6, local);
    const y = lerp(1.8, -2.2, local);
    const rot = (progress * 92 + scene * 13 + local * 18) % 360;
    const scale = 1 + Math.sin((scene + local) * 1.7) * .025;
    const alpha = scene === 0 ? lerp(.44,.66,local) : (liteFX ? .58 : .82);
    stage.style.setProperty('--fx-x', `${x.toFixed(3)}vw`);
    stage.style.setProperty('--fx-y', `${y.toFixed(3)}vh`);
    stage.style.setProperty('--fx-rot', `${rot.toFixed(2)}deg`);
    stage.style.setProperty('--fx-scale', scale.toFixed(4));
    stage.style.setProperty('--fx-alpha', alpha.toFixed(3));
  }
  ...
  function renderConfetti(scene, local) {
    ...
    confetti.style.setProperty('--confetti-travel', `${lerp(-8,132,local).toFixed(2)}vh`);
    confetti.style.setProperty('--confetti-spin', `${(local * 640).toFixed(1)}deg`);
  }
  ...
  function updateHUD(scene) {
    ...
    if (sceneIndexNumber) sceneIndexNumber.textContent = String(scene).padStart(2,'0');
  ...
  function initConfetti() {
    ...
    const count = liteFX ? 14 : 22;
    for (let i = 0; i < count; i++) {
      const p = document.createElement('i');
      p.style.setProperty('--x', `${3 + ((i * 37) % 94)}%`);
      p.style.setProperty('--c', colors[i % colors.length]);
      p.style.setProperty('--offset', `${-48 + ((i * 17) % 46)}vh`);
      p.style.setProperty('--drift', `${-24 + ((i * 31) % 49)}px`);
      p.style.setProperty('--r0', `${i * 23}deg`);
      frag.appendChild(p);
    }
  ...
  function updatePointerFX(event) {
    if (mobile || reduceMotion || liteFX) return;
    pointerTargetX = ((event.clientX / Math.max(1, innerWidth)) - .5) * 10;
    pointerTargetY = ((event.clientY / Math.max(1, innerHeight)) - .5) * 7;
    if (pointerPending) return;
    pointerPending = true;
    requestAnimationFrame(() => {
      pointerPending = false;
      stage.style.setProperty('--pointer-x', `${pointerTargetX.toFixed(2)}px`);
      stage.style.setProperty('--pointer-y', `${pointerTargetY.toFixed(2)}px`);
    });
  }

  function resetPointerFX() {
  ...
  function bind() {
    ...
    if (!mobile && !reduceMotion && !liteFX) {
      addEventListener('pointermove', updatePointerFX, { passive: true });
      document.documentElement.addEventListener('mouseleave', resetPointerFX, { passive: true });
    }
  ...
  function init() {
    ...
    document.documentElement.classList.toggle('fx-lite', liteFX);
    initConfetti();
    initContinuity();
    hydratePlate(1);
    bind();
